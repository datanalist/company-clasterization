...
def create_interactive_topic_plot(
    ...
    try:
        ...
        if (
            hasattr(dim_model, "embedding_")
            and dim_model.embedding_.shape[1] != dimensions
        ):
        ...
        logger.info(
            f"Создан интерактивный график кластеров в {dimensions}D пространстве"
        )
        return fig
...
def show_interactive_topic_plot(fig, fullscreen=True):
    ...
    try:
        ...
        logger.info("График кластеров отображен")
    except Exception as e:
        logger.error(f"Ошибка при отображении графика кластеров: {e}")
        raise
