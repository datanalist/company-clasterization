...
def start_clustering(token, date_range, ml_config=None):
    try:
...
def get_ml_models(token):
    try:
        response = requests.get(
            f"{API_URL}/api/ml/models",
            headers={"Authorization": f"Bearer {token}"}
        )
        return response.json(), response.status_code
    except Exception as e:
...
def purchase_credits(token, package_id, payment_method):
    try:
        response = requests.post(
...
def check_math_solution(token, problem_id, answer):
    try:
        response = requests.post(
            f"{API_URL}/api/credits/math-solution",
            headers={"Authorization": f"Bearer {token}"},
            json={"problem_id": problem_id, "answer": float(answer)}
        )
        return response.json(), response.status_code
    except Exception as e:
        return {"error": str(e)}, 500
...
def show_credits_page(user_info):
            ...
            for i, package in enumerate(packages):
                with cols[i % 4]:
                    with st.container():
                        ...
                        if st.button(f"Купить {package['name']}", key=f"buy_{package['id']}"):
            ...
            if selected_package:
                ...
                if st.button("Подтвердить покупку"):
                    with st.spinner("Обработка платежа..."):
                        ...
                        if status_code == 200:
                            st.success(result.get('message', 'Кредиты успешно добавлены!'))
            ...
            if "math_problem" not in st.session_state:
                ...
                if st.button("Получить задачу"):
                    with st.spinner("Генерируем задачу..."):
                        problem, status_code = get_math_problem(st.session_state["token"])
                        if status_code == 200:
                            st.session_state.math_problem = problem
                            st.rerun()
                        else:
                            st.error("Не удалось получить задачу. Попробуйте позже.")
            else:
    ...
    if status_code == 200 and history:
        ...
        if 'created_at' in history_df.columns:
            ...
            history_df['Дата'] = history_df['created_at'].dt.strftime('%Y-%m-%d %H:%M')
        ...
        if 'transaction_type' in history_df.columns:
            ...
            history_df['Тип'] = history_df['transaction_type'].map(lambda x: type_map.get(x, x))
...
def show_analytics_page(user_info):
    ...
    if status_code == 200:
        ...
        status_data = {
            ...
            "Количество": [stats.get('successful_tasks', 0), stats.get('failed_tasks', 0)]
        }
...
def show_model_selection():
    ...
    if status_code == 200:
        ...
        tab1, tab2, tab3 = st.tabs(["Модели эмбеддингов", "Модели снижения размерности", "Модели кластеризации"])
        ...
        with tab1:
            ...
            if embedding_models:
                ...
                default_model_idx = next((i for i, m in enumerate(embedding_models) if m.get("is_default")), 0)
                
                selected_embedding = st.selectbox(
                    "Выберите модель для создания эмбеддингов:",
                    embedding_models,
                    format_func=lambda x: f"{x['name']} (Дефолтная)" if x.get("is_default") else x["name"],
        ...
        with tab2:
            ...
            if reduction_models:
                # Находим дефолтную модель
                default_model_idx = next((i for i, m in enumerate(reduction_models) if m.get("is_default")), 0)
        ...
        with tab3:
            ...
            if clustering_models:
                # Находим дефолтную модель
                default_model_idx = next((i for i, m in enumerate(clustering_models) if m.get("is_default")), 0)
        ...
        if 'selected_embedding' in locals() and 'selected_reduction' in locals() and 'selected_clustering' in locals():
            return {
...
def show_main_page():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Выберите раздел:",
            ["Главная", "Мои кредиты", "Аналитика", "Настройки"]
        )
...
def show_clustering_page(user_info):
    st.markdown("<h1 class='main-header'>Кластеризация компаний по новостям</h1>", unsafe_allow_html=True)
