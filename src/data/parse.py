...
client = TelegramClient(username, api_id, api_hash, system_version="4.16.30-vxCUSTOM", device_model="Desktop", app_version="1.0")
...
async def parse_forbes_news(days=21):
    ...
    df.to_csv(f'./data/raw/forbes_news_{current_date}.csv', index=False)
    
    print(f"Собрано {len(df)} новостей за последние {days} дней")
    print(f"Данные сохранены в ./data/raw/forbes_news_{current_date}.csv")
